...
import html
import shutil
# Corrected Import: Removed execute_tests as it's a method, not a top-level import
...
import subprocess # Import subprocess for mocking in execute_tests tests
...
"""
    ...
    # --- New tests for execute_tests method (Task 1_6c_exec_tests) ---
    ...
    def test_execute_tests_success(self, mock_subprocess_run, test_driver, tmp_path, caplog):
        ...
        output, return_code = test_driver.execute_tests(test_command, cwd)
        ...
        assert "Attempting to execute test command: echo hello" in caplog.text
        assert "Test command execution completed. Return code: 0" in caplog.text
        # Corrected: Assertion should now pass as DEBUG logs are captured
        assert "Test command output:\nTest passed\n" in caplog.text # Check debug log
    ...
    def test_execute_tests_failure(self, mock_subprocess_run, test_driver, tmp_path, caplog):
        ...
        # Corrected: Set log level to DEBUG to capture the debug log message
        caplog.set_level(logging.DEBUG)
        mock_result = MagicMock(stdout="", stderr="Test failed\n", returncode=1)
        mock_subprocess_run.return_value = mock_result
        ...
        assert "Attempting to execute test command: false" in caplog.text
        assert "Test command execution completed. Return code: 1" in caplog.text
        # Corrected: Assertion should now pass as DEBUG logs are captured
        assert "Test command output:\nTest failed\n" in caplog.text # Check debug log
    ...
    def test_execute_tests_command_not_found(self, mock_subprocess_run, test_driver, tmp_path, caplog):
        ...
        assert return_code == -1
        assert "Error: Test command not found: 'nonexistent_command'" in caplog.text
    ...
    def test_execute_tests_os_error(self, mock_subprocess_run, test_driver, tmp_path, caplog):
        ...
        assert output is None
        assert return_code == -1
        assert "OS error during test command execution: permission denied" in caplog.text
    ...
    def test_execute_tests_generic_exception(self, mock_subprocess_run, test_driver, tmp_path, caplog):
        ...
        assert output is None
        assert return_code == -1
        assert "An unexpected error occurred during test execution subprocess management: unexpected error" in caplog.text

    def test_execute_tests_invalid_cwd(self, test_driver, caplog):
        ...
        # Check if subprocess.run was called - no, because the cwd check comes first
        # assert mock_subprocess_run.not_called() # This assertion needs mock_subprocess_run patched
        assert output is None
        assert return_code == -1
        assert f"Error: Working directory not found: {cwd}" in caplog.text

    # --- End new tests for execute_tests method ---
...
class LLMConfig(BaseModel):
    ...
    hugging_face_model: str = "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B"  # Default model
    max_retries: int = 3
    timeout: int = 30
    ...
    def _load_config(self) -> LLMConfig:
        try:
            ...
            return LLMConfig(
                ...
                hugging_face_model=SecureConfig.get("HUGGING_FACE_MODEL", "Qwen/QwQ-32B"),  # Load model from .env or use default
    ...
    def _configure_providers(self):
        if self.config.provider == LLMProvider.GEMINI:
            ...
            self.client = InferenceClient(
                ...
                model=self.config.hugging_face_model,  # Use the model from config
            )
        else:
...
class EnhancedLLMOrchestrator(LLMOrchestrator):
    ...
    ):  # Add spec and ethics_engine, use forward ref
